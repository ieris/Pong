...
class sys_db_Manager {
	...
	public function unsafeGet($id, $lock = null) {
		...
		$tmp1 = Std::string($id);
		$x = $this->getFromCacheKey(_hx_string_or_null($tmp1) . _hx_string_or_null($this->table_name));
		$tmp2 = null;
		if($x !== null) {
			if(!(!$lock)) {
				$tmp2 = $x->_lock;
		...
		$s->add($this->table_name);
		$s->add(" WHERE ");
		$tmp3 = $this->table_keys[0];
		$tmp4 = $this->quoteField($tmp3);
		$s->add($tmp4);
		$s->add(" = ");
	...
	public function h__get($x, $prop, $key, $lock) {
		...
		$tmp = Reflect::field($x, $key);
		$y = $this->unsafeGet($tmp, $lock);
		$x->{$prop} = $v;
		return $y;
	}
	public function h__set($x, $prop, $key, $v) {
		...
		if($v === null) {
			...
			$tmp = $this->table_keys[0];
			$value = Reflect::field($v, $tmp);
			$x->{$key} = $value;
		}
		return $v;
	}
